...
const SocialShare = ({ letterId, url, title }) => {
  const shareButtons = [
    {
      platform: 'facebook',
      icon: (
    ...
    },
    {
      ...
      shareUrl: url // Instagram doesn't support direct sharing, but we'll track it
    }
  ];
  ...
  const handleShare = async (platform, shareUrl) => {
    try {
      // Track share
      await axios.post(`/api/letters/${letterId}/share`, { platform });
      
      // Open share dialog
      if (platform === 'instagram') {
        // For Instagram, we can only copy the URL
        await navigator.clipboard.writeText(url);
        alert('URL copied! You can now share it on Instagram');
      } else {
  ...
  return (
    <div className="flex space-x-4">
      {shareButtons.map(({ platform, icon, shareUrl }) => (
        <button
          key={platform}
